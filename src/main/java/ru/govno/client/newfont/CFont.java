...
public class CFont {
    ...
    public void drawChar2(CharData[] chars, char c, float x, float y, int color) throws ArrayIndexOutOfBoundsException {
        try {
            this.drawQuad2(x, y, chars[c].width, chars[c].height, chars[c].storedX, chars[c].storedY, chars[c].width, chars[c].height, color);
        }
        catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected void drawQuad2(float x, float y, float width, float height, float srcX, float srcY, float srcWidth, float srcHeight, int color) {
        this.bufferBuilder.pos(x + width, y).tex((srcX /= 512.0f) + (srcWidth /= 512.0f), srcY /= 512.0f).color(color).endVertex();
        this.bufferBuilder.pos(x, y).tex(srcX, srcY).color(color).endVertex();
        this.bufferBuilder.pos(x, y + height).tex(srcX, srcY + (srcHeight /= 512.0f)).color(color).endVertex();
        this.bufferBuilder.pos(x + width, y).tex(srcX + srcWidth, srcY).color(color).endVertex();
        this.bufferBuilder.pos(x, y + height).tex(srcX, srcY + srcHeight).color(color).endVertex();
        this.bufferBuilder.pos(x + width, y + height).tex(srcX + srcWidth, srcY + srcHeight).color(color).endVertex();
    }
