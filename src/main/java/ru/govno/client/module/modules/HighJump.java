...
extends Module {
    ...
    public HighJump() {
        ...
        this.GroundJump = new BoolSettings("GroundJump", true, this);
        this.settings.add(this.GroundJump);
        this.JumpMode = new ModeSettings("JumpMode", "MatrixNew", this, new String[]{"Normal", "MatrixOld", "MatrixNew", "MatrixWait", "MatrixDestruct"}, () -> this.GroundJump.getBool());
        this.settings.add(this.JumpMode);
        this.JumpPulse = new FloatSettings("JumpPulse", 1.0f, 10.0f, 0.0f, this, () -> this.GroundJump.getBool() && (this.JumpMode.currentMode.equalsIgnoreCase("Normal") || this.JumpMode.currentMode.equalsIgnoreCase("MatrixWait")));
        this.settings.add(this.JumpPulse);
        this.MotionY = new FloatSettings("MotionY", 3.0f, 10.0f, 0.6f, this, () -> this.GroundJump.getBool() && (this.JumpMode.currentMode.equalsIgnoreCase("MatrixNew") || this.JumpMode.currentMode.equalsIgnoreCase("MatrixDestruct")));
        this.settings.add(this.MotionY);
        this.WaterJump = new BoolSettings("WaterJump", false, this);
        this.settings.add(this.WaterJump);
        this.WaterJumpMode = new ModeSettings("WaterJumpMode", "Default", this, new String[]{"Default", "StormHVH"}, () -> this.WaterJump.getBool());
        this.settings.add(this.WaterJumpMode);
        this.WaterPulse = new FloatSettings("WaterPulse", 10.0f, 16.0f, 0.0f, this, () -> this.WaterJump.getBool());
        this.settings.add(this.WaterPulse);
        this.WaterYPort = new BoolSettings("WaterYPort", true, this, () -> this.WaterJump.getBool() && !this.WaterJumpMode.getMode().equalsIgnoreCase("StormHVH"));
        this.settings.add(this.WaterYPort);
        this.CactusJump = new BoolSettings("CactusJump", true, this);
        this.settings.add(this.CactusJump);
        this.CactusPulse = new FloatSettings("CactusPulse", 3.0f, 10.0f, 0.0f, this, () -> this.CactusJump.getBool());
        this.settings.add(this.CactusPulse);
        this.CactusVastum = new BoolSettings("CactusVastum", false, this, () -> this.CactusJump.getBool());
        this.settings.add(this.CactusVastum);
        this.WebJump = new BoolSettings("WebJump", true, this);
        this.settings.add(this.WebJump);
        this.WebPulse = new FloatSettings("WebPulse", 3.0f, 10.0f, 0.0f, this, () -> this.WebJump.getBool());
        this.settings.add(this.WebPulse);
        this.WebBoost = new BoolSettings("WebBoost", false, this, () -> this.WebJump.getBool());
        this.settings.add(this.WebBoost);
        this.SelfWebPlace = new BoolSettings("SelfWebPlace", true, this, () -> this.WebJump.getBool());
    ...
    public void onPacketReceive(EventReceivePacket event) {
        ...
        if (this.GroundJump.getBool() && this.JumpMode.currentMode.equalsIgnoreCase("MatrixWait") && (packet2 = event.getPacket()) instanceof SPacketChat && ((packet = (SPacketChat)packet2).getChatComponent().getUnformattedText().contains("\u0418\u0437\u0432\u0438\u043d\u0438\u0442\u0435, \u043d\u043e \u0432\u044b \u043d\u0435 \u043c\u043e\u0436\u0435\u0442\u0435") || packet.getChatComponent().getUnformattedText().contains("but you can't"))) {
            this.toDo = true;
    ...
    void groundJump() {
        ...
        if (this.JumpMode.currentMode.equalsIgnoreCase("MatrixWait")) {
            ...
            if (this.state != null && has && (Minecraft.player.onGround || JesusSpeed.isJesused) || this.toDo && Minecraft.player.onGround) {
                Minecraft.player.motionY = this.JumpPulse.getFloat();
                MoveMeHelp.setSpeed(0.0);
                MoveMeHelp.setCuttingSpeed(0.0);
                Minecraft.player.jumpMovementFactor = 0.0f;
                this.wait.reset();
            ...
            for (float x = -5.0f; x < 5.0f; x += 1.0f) {
                for (float y = -5.0f; y < 1.21f; y += 1.0f) {
    ...
    void cactusJump() {
        if (BlockCactus.canLeave && Minecraft.player.hurtTime > 0) {
            if (this.CactusVastum.getBool() && Minecraft.player.onGround) {
                ...
                Minecraft.player.setPosition(Minecraft.player.posX + (double)1.0E-6f, Minecraft.player.posY + (double)(1000.0f * (this.CactusPulse.getFloat() / 10.0f)), Minecraft.player.posZ + (double)1.0E-6f);
                Minecraft.player.motionY = 0.0;
            } else {
                Minecraft.player.motionY = this.CactusPulse.getFloat();
            }
    ...
    void webLeave() {
        ...
        if ((HighJump.mc.world.getBlockState(new BlockPos(x, y - (double)(ex = 0.2f), z)).getBlock() == Blocks.WEB || HighJump.mc.world.getBlockState(new BlockPos(x - (double)0.3f, y - (double)ex, z)).getBlock() == Blocks.WEB || HighJump.mc.world.getBlockState(new BlockPos(x, y - (double)ex, z - (double)0.3f)).getBlock() == Blocks.WEB || HighJump.mc.world.getBlockState(new BlockPos(x - (double)0.3f, y - (double)ex, z - (double)0.3f)).getBlock() == Blocks.WEB || HighJump.mc.world.getBlockState(new BlockPos(x + (double)0.3f, y - (double)ex, z)).getBlock() == Blocks.WEB || HighJump.mc.world.getBlockState(new BlockPos(x, y - (double)ex, z + (double)0.3f)).getBlock() == Blocks.WEB || HighJump.mc.world.getBlockState(new BlockPos(x + (double)0.3f, y - (double)ex, z + (double)0.3f)).getBlock() == Blocks.WEB || HighJump.mc.world.getBlockState(new BlockPos(x + (double)0.3f, y - (double)ex, z - (double)0.3f)).getBlock() == Blocks.WEB || HighJump.mc.world.getBlockState(new BlockPos(x - (double)0.3f, y - (double)ex, z + (double)0.3f)).getBlock() == Blocks.WEB) && HighJump.mc.world.getBlockState(new BlockPos(x, y, z)).getBlock() != Blocks.WEB && Minecraft.player.fallDistance == 0.0f && !Minecraft.player.isInWeb && Minecraft.player.motionY < 0.0) {
            Minecraft.player.motionY = this.WebPulse.getFloat();
    ...
    public boolean waterLeaveCanSolid() {
        return Minecraft.player != null && get != null && this.actived && this.WaterJump.getBool() && (Minecraft.player == null || !Minecraft.player.isInLiquid2()) && !this.WaterJumpMode.getMode().equalsIgnoreCase("StormHVH");
    }
    ...
    void waterLeaveStorm() {
        ...
        double y = Minecraft.player.posY;
        double z = Minecraft.player.posZ;
        if (HighJump.mc.world.getBlockState(new BlockPos(x, y, z)).getBlock() instanceof BlockLiquid) {
            double d = Minecraft.player.motionY = Minecraft.player.isJumping() || Minecraft.player.motionY > 0.01 ? 4.0 : -8.0;
            if (Minecraft.player.onGround) {
                Minecraft.player.motionY = 0.6f;
                this.toggle();
            }
            if (Minecraft.player.motionY > 1.0 && HighJump.mc.world.getBlockState(new BlockPos(x, y + 3.0, z)).getBlock() instanceof BlockLiquid) {
                this.toggle();
            }
        }
    }
    ...
    public void onToggled(boolean actived) {
        ...
        BlockCactus.canLeave = false;
        if (!actived && this.GroundJump.getBool() && this.JumpMode.currentMode.equalsIgnoreCase("MatrixOld") && HighJump.mc.timer.speed == (double)1.45f) {
            HighJump.mc.timer.speed = 1.0;
        }
