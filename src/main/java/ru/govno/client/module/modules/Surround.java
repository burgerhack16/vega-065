...
extends Module {
    ...
    public Surround() {
        ...
        this.AutoCenter = new BoolSettings("AutoCenter", true, this);
        this.settings.add(this.AutoCenter);
        this.PlaceHand = new ModeSettings("PlaceHand", "Auto", this, new String[]{"OffHand", "MainHand", "Auto"});
        this.settings.add(this.PlaceHand);
        this.Mode = new ModeSettings("Mode", "Fast", this, new String[]{"Fast", "Queue"});
        this.settings.add(this.Mode);
        this.Switch = new ModeSettings("Switch", "Client", this, new String[]{"Client", "Silent"});
        this.settings.add(this.Switch);
        this.PlaceMS = new FloatSettings("PlaceMS", 100.0f, 250.0f, 0.0f, this);
        this.settings.add(this.PlaceMS);
        this.PostPlacing = new BoolSettings("PostPlacing", true, this, () -> this.Mode.getMode().equalsIgnoreCase("Queue"));
    ...
    private List<BlockPos> getCurPoses() {
        ...
        Arrays.stream(EnumFacing.HORIZONTALS).filter(face -> this.getBlockMaterial(myPos.down().add(face.getDirectionVec())).isReplaceable() && BlockUtils.canPlaceBlock(myPos.down().add(face.getDirectionVec()))).forEach(face -> cur.add(myPos.down().add(face.getDirectionVec())));
        Arrays.stream(EnumFacing.HORIZONTALS).filter(face -> this.getBlockMaterial(myPos.add(face.getDirectionVec())).isReplaceable() && BlockUtils.canPlaceBlock(myPos.add(face.getDirectionVec()))).forEach(face -> cur.add(myPos.add(face.getDirectionVec())));
        return cur;
    }

    private void updatePoses() {
        toPlacePoses = this.getCurPoses().stream().filter(temp -> this.canPlaceObsidian((BlockPos)temp, this.IgnoreWalls.getBool())).collect(Collectors.toList());
    }
    ...
    public void onUpdate() {
        if (this.canPlace()) {
            ...
            if (toPlacePoses != null && toPlacePoses.size() > 0 && this.delayTimer.hasReached(this.PlaceMS.getFloat())) {
                this.handSlotSnapFor(this.getUsedHand(), this.switchIsSilent(), () -> {
                    ...
                    if (isFast) {
                        toPlacePoses.forEach(pos -> this.placeBlock((BlockPos)pos, this.getUsedHand()));
