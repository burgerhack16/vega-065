...
extends Module {
    ...
    FloatSettings BlockCap = new FloatSettings("BlockCap", 128.0f, 256.0f, 8.0f, this);
    ...
    public InvMngr() {
        ...
        this.settings.add(this.BlockCap);
        this.SortDelay = new FloatSettings("SortDelay", 50.0f, 250.0f, 0.0f, this);
        this.settings.add(this.SortDelay);
        this.ForTheArcher = new BoolSettings("ForTheArcher", false, this);
        this.settings.add(this.ForTheArcher);
        this.Food = new BoolSettings("Food", false, this);
        this.settings.add(this.Food);
        this.Sword = new BoolSettings("Sword", true, this);
        this.settings.add(this.Sword);
        this.InvCleaner = new BoolSettings("InvCleaner", true, this);
        this.settings.add(this.InvCleaner);
        this.OnlyInInv = new BoolSettings("OnlyInInv", true, this);
        this.settings.add(this.OnlyInInv);
        this.NoMovingSwap = new BoolSettings("NoMovingSwap", false, this);
        this.settings.add(this.NoMovingSwap);
    }
    ...
    public void onUpdate() {
        ...
        if (!(InvMngr.mc.currentScreen instanceof GuiInventory) && this.OnlyInInv.getBool()) {
            return;
        }
        ...
        if (InvMngr.mc.currentScreen == null || InvMngr.mc.currentScreen instanceof GuiInventory || InvMngr.mc.currentScreen instanceof GuiChat) {
            ...
            if (this.timer.hasReached(delay) && this.InvCleaner.getBool()) {
                for (int i = 9; i < 45; ++i) {
                    ItemStack is;
    ...
    public boolean isBestWeapon(ItemStack stack) {
        ...
        for (int i = 9; i < 45; ++i) {
            ItemStack is;
            if (!Minecraft.player.inventoryContainer.getSlot(i).getHasStack() || !(this.getDamage(is = Minecraft.player.inventoryContainer.getSlot(i).getStack()) > damage) || !(is.getItem() instanceof ItemSword) && this.Sword.getBool()) continue;
            return false;
        }
    ...
    public void getBestWeapon(int slot) {
        for (int i = 9; i < 45; ++i) {
            ItemStack is;
            if (!Minecraft.player.inventoryContainer.getSlot(i).getHasStack() || !this.isBestWeapon(is = Minecraft.player.inventoryContainer.getSlot(i).getStack()) || !(this.getDamage(is) > 0.0f) || !(is.getItem() instanceof ItemSword) && this.Sword.getBool()) continue;
            this.swap(i, slot - 36);
            this.timer.reset();
            break;
        }
    ...
    public boolean shouldDrop(ItemStack stack, int slot) {
        ...
        if (stack.getItem() instanceof ItemBlock && ((float)this.getBlockCount() > this.BlockCap.getFloat() || invalidBlocks.contains(((ItemBlock)stack.getItem()).getBlock()))) {
            return true;
        }
        if (stack.getItem() instanceof ItemPotion && this.isBadPotion(stack)) {
            return true;
        }
        if (stack.getItem() instanceof ItemFood && this.Food.getBool() && !(stack.getItem() instanceof ItemAppleGold)) {
            return true;
        }
        ...
        if ((stack.getItem() instanceof ItemBow || stack.getItem().getUnlocalizedName().contains("arrow")) && this.ForTheArcher.getBool()) {
            return true;
        }
    ...
    private void getBestPickaxe() {
        for (int i = 9; i < 45; ++i) {
            ...
            if (!Minecraft.player.inventoryContainer.getSlot(pickaxeSlot).getHasStack()) {
                ...
                this.timer.reset();
                if (!(this.SortDelay.getFloat() > 0.0f)) continue;
                return;
            }
