...
extends Module {
    ...
    public Scaffold() {
        ...
        this.Delay = new FloatSettings("Delay", 100.0f, 500.0f, 0.0f, this);
        this.settings.add(this.Delay);
        this.BlockCount = new BoolSettings("BlockCount", true, this);
        this.settings.add(this.BlockCount);
        this.Sprint = new BoolSettings("Sprint", false, this);
        this.settings.add(this.Sprint);
        this.Sneak = new ModeSettings("Sneak", "Packet", this, new String[]{"Packet", "Client", "None"});
        this.settings.add(this.Sneak);
        this.Swing = new BoolSettings("Swing", false, this);
        this.settings.add(this.Swing);
        this.TowerMotion = new BoolSettings("TowerMotion", true, this);
        this.settings.add(this.TowerMotion);
        this.Tower = new ModeSettings("Tower", "Matrix", this, new String[]{"Matrix", "NCP", "Strict", "Matrix&AAC"}, () -> this.TowerMotion.getBool());
        this.settings.add(this.Tower);
        this.SpeedI = new FloatSettings("Speed", 0.58f, 1.0f, 0.01f, this);
        this.settings.add(this.SpeedI);
        this.TimerUse = new BoolSettings("TimerUse", true, this);
        this.settings.add(this.TimerUse);
        this.TimerWalk = new FloatSettings("TimerWalk", 1.0f, 2.0f, 0.5f, this, () -> this.TimerUse.getBool());
        this.settings.add(this.TimerWalk);
        this.TimerTower = new FloatSettings("TimerTower", 1.8f, 2.0f, 0.5f, this, () -> this.TimerUse.getBool());
        this.settings.add(this.TimerTower);
        this.SilentSwitch = new BoolSettings("SilentSwitch", false, this);
        this.settings.add(this.SilentSwitch);
        get = this;
    }
    ...
    public void onToggled(boolean actived) {
        if (actived) {
            ...
            this.delayUtils.reset();
            this.placePossibilities.clear();
            Scaffold.mc.gameSettings.keyBindSneak.pressed = Keyboard.isKeyDown((int)Scaffold.mc.gameSettings.keyBindSneak.getKeyCode());
            if (this.sneaking) {
                this.sneaking = false;
    ...
    public void onUpdate() {
        boolean isTowered;
        boolean bl = isTowered = this.TowerMotion.getBool() && Minecraft.player.isJumping() && blockCount > 0;
        if (this.TimerUse.getBool()) {
            Scaffold.mc.timer.speed = isTowered ? (double)this.TimerTower.getFloat() : (double)this.TimerWalk.getFloat();
            Timer.forceTimer((float)Scaffold.mc.timer.speed);
        }
        ...
        if (this.TowerMotion.getBool() && blockCount > 0) {
            ...
            if (this.Tower.getMode().equalsIgnoreCase("Strict") && Minecraft.player.isJumping() && !MoveMeHelp.moveKeysPressed() && Minecraft.player.onGround) {
                ...
                if (slotIndex != -1) {
                    ...
                    Minecraft.player.inventory.currentItem = this.SilentSwitch.getBool() ? slotIndex : slotIndex;
                    this.placeAction();
            ...
            if (this.Tower.getMode().equalsIgnoreCase("NCP") && Scaffold.mc.gameSettings.keyBindJump.isKeyDown()) {
