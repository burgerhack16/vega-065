...
extends Module {
    ...
    public DeathBox() {
        ...
        this.LifeTime = new FloatSettings("LifeTime", 15000.0f, 20000.0f, 2000.0f, this);
    ...
    private Vector3d project2D(int scaleFactor, float x, float y, float z) {
        ...
        return GLU.gluProject((float)x, (float)y, (float)z, (FloatBuffer)this.modelview, (FloatBuffer)this.projection, (IntBuffer)this.viewport, (FloatBuffer)this.vector) ? new Vector3d((double)(this.vector.get(0) / (float)scaleFactor), (double)(((float)Display.getHeight() - this.vector.get(1)) / (float)scaleFactor), (double)this.vector.get(2)) : null;
    }
    ...
    private AxisAlignedBB getEntityBox(Entity entity) {
        ...
        } else if (entity != null && (aabb = entity.getEntityBoundingBox()) != null) {
    ...
    private void drawDeathBoxTag(Box box, float alphaPC, ScaledResolution sr) {
        ...
        if (this.nameTagVecSetSuccess(pos)) {
            ...
            GL11.glPointSize((float)(alphaPC * 2.5f + 1.0E-4f));
            GL11.glBegin((int)0);
