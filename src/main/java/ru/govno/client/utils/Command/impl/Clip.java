...
extends Command {
    ...
    public void onCommand(String[] args) {
        try {
            ...
            boolean bl = elytra = Bypass.get.isActived() && Bypass.get.ElytraClip.getBool();
            if (elytra) {
    ...
    public static void goClip(double y, double h, boolean canElytra) {
        boolean canBreak;
        boolean bl = canBreak = Bypass.get.isActived() && Bypass.get.BreakingClip.getBool();
        if (canBreak) {
            BlockPos pos = null;
        ...
        int de = (int)(Math.abs(y / 10.0) + Math.abs(h) * 0.25);
