        ...
        try {
            for (Module f : Client.moduleManager.getModuleList()) {
                if (args[1].equalsIgnoreCase("get") && f.getName().equalsIgnoreCase(args[2])) {
                    Client.msg("\u00a7b\u00a7lBinds:\u00a7r \u00a77[\u00a77\u00a7l" + f.getName() + "\u00a7r\u00a77] key: " + Keyboard.getKeyName((int)f.getBind()) + ".", false);
                    continue;
                }
                ...
                if (args[0].equalsIgnoreCase("bind") || args[0].equalsIgnoreCase("bnd") || args[0].equalsIgnoreCase("b")) {
                    ...
                    if (args[2].equalsIgnoreCase("null") || args[2].equalsIgnoreCase("none") || args[2].equalsIgnoreCase("0")) {
                    ...
                    } else if (f.getBind() == Keyboard.getKeyIndex((String)args[2].toUpperCase())) {
                        ...
                        f.setBind(Keyboard.getKeyIndex((String)args[2].toUpperCase()));
                    }
                    return;
                }
                if (!args[0].equalsIgnoreCase("unbind") && !args[0].equalsIgnoreCase("unb")) continue;
                if (args[1].equalsIgnoreCase("all")) {
                    List<Module> toUnbind = Client.moduleManager.getModuleList().stream().filter(m -> m.getBind() != 0 && !m.getName().equalsIgnoreCase("ClickGui")).collect(Collectors.toList());
                    if (toUnbind.isEmpty()) {
