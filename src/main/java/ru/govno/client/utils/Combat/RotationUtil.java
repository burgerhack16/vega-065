...
public class RotationUtil {
    ...
    public static float calcYawOffset(float yaw) {
        ...
        float offset = renderYawOffset = Minecraft.player.renderYawOffset;
    ...
    public static float[] getRots(Entity entityIn, boolean random, boolean turnAwayBoundingBox, boolean alwaysMultipoints) {
        ...
        try {
            rotate = RotationUtil.getNeededFacing(to, random && !turnAwayBoundingBox, Minecraft.player, turnAwayBoundingBox);
        }
        catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static float[] getRots2(Entity entityIn, boolean random, boolean turnAwayBoundingBox, boolean alwaysMultipoints) {
        return RotationUtil.getNeededFacing2((EntityLivingBase)entityIn, random && !turnAwayBoundingBox, turnAwayBoundingBox, alwaysMultipoints);
    }
    ...
    public static float[] getRots3(Entity entityIn, boolean random, boolean turnAwayBoundingBox, boolean alwaysMultipoints) {
        ...
        float[] rotate = new float[]{Yaw, Pitch};
        try {
            rotate = RotationUtil.getNeededFacingNCP(to, random && !turnAwayBoundingBox, Minecraft.player, turnAwayBoundingBox);
        }
        catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static float[] getMatrixRots2(Entity e, float range) {
        double diffZ;
        double diffX;
        double raz = MathUtils.clamp(Minecraft.player.posY + (double)Minecraft.player.getEyeHeight() - e.posY, (double)e.height * 0.3, (double)Math.min(Minecraft.player.getEyeHeight(), e.height * 0.8f));
    ...
    public static float[] getVecNeeded(Vec3d vec, Vec3d rotateAt) {
        ...
        return new float[]{(float)Math.atan2(diffs[2], diffs[0]) * 57.29578f - 90.0f, (float)Math.atan2(diffs[1], diffs[3]) * -57.29578f};
    }
    ...
    public static float[] getNeededFacingNCP(Vec3d vec, boolean randomizer, Entity rotateAt, boolean turnAwayBoundingBox) {
        ...
        if (randomizer) {
            float speedYaw = Math.min(RotationUtil.getAngleDifference(Yaw, yaw) / 180.0f, 1.0f);
            speedYaw = (float)MathUtils.easeOutCubic(speedYaw);
            float speedPitch = 5.14f + 4.2f * speedYaw;
            Yaw = GCDFix.getFixedRotation(Yaw + MathHelper.clamp(MathHelper.wrapDegrees(yaw - Yaw), -(speedYaw *= 40.0f), speedYaw));
            Pitch = GCDFix.getFixedRotation(Pitch + (float)MathHelper.clamp(MathHelper.clamp((double)pitch, -89.5, 89.5) - (double)Pitch, (double)(-speedPitch), (double)speedPitch));
        }
        return new float[]{randomizer ? Yaw : yaw, randomizer ? Pitch : pitch};
    }
    ...
    public static float[] getNeededFacing2(EntityLivingBase target, boolean randomizer, boolean turnAwayBoundingBox, boolean alwaysMultipoints) {
        ...
        yaw = Yaw + RotationUtil.getFixedRotation(MathUtils.wrapAngleTo180_float(yaw - Yaw - (0.5f + (float)Math.random()) * yRand));
    ...
    public static float getAngle(Entity entity) {
        ...
        return (float)((double)yaw - RotationUtil.Interpolate(Minecraft.player.rotationYaw, Minecraft.player.prevRotationYaw, 1.0));
    }
