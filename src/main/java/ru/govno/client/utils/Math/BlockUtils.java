...
public class BlockUtils {
    ...
    public static boolean blockMaterialIsPlacelable(BlockPos pos) {
        return !BlockUtils.getBlockMaterial(pos).isReplaceable() && !BlockUtils.getBlockMaterial(pos).isLiquid() && BlockUtils.getBlockMaterial(pos).isSolid();
    }
    ...
    public static BlockPos currentWithSideSetsPos(BlockPos pos) {
        if (BlockUtils.blockMaterialIsPlacelable(pos.east())) {
            return pos.east();
        }
        if (BlockUtils.blockMaterialIsPlacelable(pos.west())) {
            return pos.west();
        }
        if (BlockUtils.blockMaterialIsPlacelable(pos.south())) {
            return pos.south();
        }
        if (BlockUtils.blockMaterialIsPlacelable(pos.north())) {
            return pos.north();
        }
        if (BlockUtils.blockMaterialIsPlacelable(pos.up())) {
            return pos.up();
        }
        if (BlockUtils.blockMaterialIsPlacelable(pos.down())) {
            return pos.down();
        }
        return null;
    }
    ...
    public static boolean canPosBeSeenEntityWithCustomVec(Vec3d pos, Entity entityIn, Vec3d entityVirtPos, bodyElement bodyElement2) {
        double w = (double)entityIn.width / 2.0;
    ...
    public static EnumFacing getPlaceableSideSeen(BlockPos pos, Entity self) {
        for (EnumFacing facing : new EnumFacing[]{EnumFacing.EAST, EnumFacing.WEST, EnumFacing.SOUTH, EnumFacing.NORTH, EnumFacing.UP, EnumFacing.DOWN}) {
            ...
            IBlockState state;
            BlockPos offset = pos.offset(facing);
            if (!BlockUtils.mc.world.getBlockState(offset).getBlock().canCollideCheck(BlockUtils.mc.world.getBlockState(offset), false) || (state = BlockUtils.mc.world.getBlockState(offset)).getMaterial().isReplaceable() || !self.canEntityBeSeenVec3d(placeFaceVec = new Vec3d(pos).addVector(0.5, 0.5, 0.5).addVector((double)facing.getFrontOffsetX() * 0.5, (double)facing.getFrontOffsetY() * 0.5, (double)facing.getFrontOffsetZ() * 0.5))) continue;
            return facing;
        }
        return null;
    }
