...
extends Manager<Config> {
    ...
    private static ArrayList<Config> loadConfigs() {
        ...
        if (files != null) {
            for (File file : files) {
                if (!FilenameUtils.getExtension((String)file.getName()).equals(format) || FilenameUtils.removeExtension((String)file.getName()).equalsIgnoreCase("nulled")) continue;
    ...
    public void load() {
        ...
        if (configDirectory != null) {
            File[] files;
            for (File f2 : files = configDirectory.listFiles(f -> !f.isDirectory() && FilenameUtils.getExtension((String)f.getName()).equals(format))) {
