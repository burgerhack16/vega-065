    ...
    public void drawScreen(float x, float y, int step, int mouseX, int mouseY, float partialTicks) {
        ...
        if (scaledAlphaPercent * 255.0f >= 33.0f) {
            ...
            if ((175.0f - 175.0f * MathUtils.clamp(this.changeAnim.anim / 4.0f, 0.0f, 1.0f)) * scaledAlphaPercent >= 33.0f) {
                Fonts.comfortaaBold_12.drawString(this.setting.currentMode, x + 11.0f + (float)Fonts.comfortaaBold_12.getStringWidth(this.setting.getName()) + this.changeAnim.anim, y + 8.5f + this.changeAnim.anim / 4.0f * (float)(this.changeAnim.to == 0.0f ? -1 : 1), ColorUtils.swapAlpha(-1, MathUtils.clamp((175.0f - 175.0f * MathUtils.clamp(this.changeAnim.anim / 4.0f, 0.0f, 1.0f)) * scaledAlphaPercent, 26.0f, 175.0f)));
            }
        }
        ...
        if (getHeight > 18.5f) {
            for (String mode : this.setting.modes) {
                ...
                if (!((height += 13.0f) < getHeight)) continue;
                ...
                RenderUtils.drawAlphedRect(x + 6.5f, y + height + 6.5f, x + this.getWidth() - 6.5f, y + height + 7.0f, rectCol2);
                RenderUtils.drawAlphedRect(x + 6.0f, y + height - 5.0f, x + 6.5f, y + height - 4.5f, rectCol2);
                RenderUtils.drawAlphedRect(x + this.getWidth() - 6.5f, y + height - 5.0f, x + this.getWidth() - 6.0f, y + height - 4.5f, rectCol2);
                ...
                if (mode.equalsIgnoreCase(this.setting.currentMode)) {
                    ...
                    float yn = y + height - (float)(mode.equalsIgnoreCase(this.setting.currentMode) ? 1 : 0) - this.changeAnim.anim / 2.5f;
                    ...
                    for (char c : mode.toCharArray()) {
                        ...
                        if (ColorUtils.getAlphaFromColor(col1 = ColorUtils.swapAlpha(col1, (float)ColorUtils.getAlphaFromColor(col1) * (scaledAlphaPercent * this.anim.anim))) >= 32) {
                            modedFont.drawString(String.valueOf(c), (double)((float)modedFont.getStringWidth(mode) + index + xn) + 0.5, (double)yn + 0.5, ColorUtils.swapDark(col1, ColorUtils.getBrightnessFromColor(col1) / 3.0f));
                ...
                modedFont.drawStringWithShadow(mode, x + this.getWidth() / 2.0f - width, y + height - (float)(mode.equalsIgnoreCase(this.setting.currentMode) ? 1 : 0), ColorUtils.swapAlpha(ColorUtils.getColor(175), MathUtils.clamp(175.0f * (this.anim.anim / 2.0f + 0.5f) * scaledAlphaPercent, 26.0f, 255.0f)));
            }
        }
