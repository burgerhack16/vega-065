...
extends Set {
    ...
    public void drawScreen(float x, float y, int step, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtils.drawRoundedFullGradientShadowFullGradientRoundedFullGradientRectWithBloomBool(posX, posY + 0.5f, posX + w, posY + getHeight - 2.0f, 2.0f, 0.5f, cc, cc, cc, cc, false, true, true);
        RenderUtils.drawAlphedSideways(posX, posY, posX + w, posY + 1.0f, ColorUtils.swapAlpha(ClickGuiScreen.getColor((int)((float)step + y / this.getHeight()), this.setting.module.category), 255.0f * scaledAlphaPercent), ColorUtils.swapAlpha(ClickGuiScreen.getColor((int)((float)(step + 120) + y / getHeight), this.setting.module.category), 255.0f * scaledAlphaPercent));
        ...
        this.drawArrow(posX + w - 8.0f, posY + 5.0f, ColorUtils.swapAlpha(ColorUtils.getFixedWhiteColor(), MathUtils.clamp(26.0f * scaledAlphaPercent + 175.0f * this.anim.anim * scaledAlphaPercent, 0.0f, 255.0f)));
        ...
        for (int i2 = 0; i2 < 12; i2 += 2) {
            for (int i = 0; i < 8; i += 2) {
                GL11.glTranslated((double)(x + 8.5f), (double)(y + 6.0f), (double)0.0);
    ...
    public void mouseClicked(int x, int y, int mouseX, int mouseY, int mouseButton) {
        ...
        if ((mouseButton == 1 || mouseButton == 0) && this.ishover(x + 5, y + 5, (float)x + this.getWidth() - 4.0f, y + 20, mouseX, mouseY)) {
